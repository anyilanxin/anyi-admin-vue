...
export function useRootSetting() {
  ...
  const getOpenKeepAlive = computed(() => configStore.getProjectConfig.openKeepAlive)

  const getSettingButtonPosition = computed(
    () => configStore.getProjectConfig.settingButtonPosition,
  )

  const getCanEmbedIFramePage = computed(() => configStore.getProjectConfig.canEmbedIFramePage)

  const getPermissionMode = computed(() => configStore.getProjectConfig.permissionMode)
  ...
  const getUseOpenBackTop = computed(() => configStore.getProjectConfig.useOpenBackTop)

  const getShowSettingButton = computed(() => configStore.getProjectConfig.showSettingButton)

  const getShowFooter = computed(() => configStore.getProjectConfig.showFooter)

  const getShowBreadCrumb = computed(() => configStore.getProjectConfig.showBreadCrumb)

  const getThemeColor = computed(() => configStore.getProjectConfig.themeColor)

  const getShowBreadCrumbIcon = computed(() => configStore.getProjectConfig.showBreadCrumbIcon)
  ...
  const getShowDarkModeToggle = computed(() => configStore.getProjectConfig.showDarkModeToggle)
