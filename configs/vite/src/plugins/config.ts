...
import { getAppConfigFileName } from '@anyi/coreutils'
...
function createConfig(params: Options) {
  ...
  try {
    ...
    console.log(cyan(`✨ [${APP_NAME}]`) + ` - configuration file is build successfully:`)
    console.log(gray(OUTPUT_DIR + '/' + green(configFileName)) + '\n')
  } catch (error) {
    console.log(red('configuration file configuration file failed to package:\n' + error))
  }
}
