...
<template>
  <template v-if="menuItem?.children && menuItem?.children.length !== 0">
    <a-sub-menu :key="menuItem.name">
      <template #icon>
        <div
          :class="[showCustom && 'anyi-menu-sub-item', isDark && 'anyi-menu-sub-item-dark']"
          v-if="
            sidebar.collapsed &&
      ...
      <template #title>
        <span :class="[showCustom && 'anyi-menu-sub-item', isDark && 'anyi-menu-sub-item-dark']">{{
          $t(menuItem?.title || '')
        }}</span>
      </template>
      <SubMenu
...
const showCustom = computed(() => {
  return props.mode == MenuModeEnum.VERTICAL && !unref(isDark)
})
const goto = (item: any) => {
  // Open external link
