...
import { isUrl, filterTree } from '@anyi/coreutils'
...
async function getAsyncMenus() {
  ...
  if (isBackMode()) {
    return authStore.getBackMenuList.filter((item) => !item.meta?.hideMenu && !item.hideMenu)
  }
  return authStore.getFrontMenuList.filter((item) => !item.hideMenu && !item.meta?.hideMenu)
}
