 ...
 * @description: 退出登录
 */
export const loginOut = () => request.get<string>({ url: SERVICE_API_PREFIX.AUTH + '/logout' })
...
export const kickOut = (loginUnique: string) =>
  request.post<string>({
    url: SERVICE_API_PREFIX.AUTH + `/kick-out/${loginUnique}`,
  })
...
export const getPictureCode = () =>
  request.get<ValidateDto>({ url: SERVICE_API_PREFIX.AUTH + '/code/picture' })
