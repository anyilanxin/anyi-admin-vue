...
<template>
  <a-input :size="size" :class="prefixCls">
    <template #prefix>
      <slot name="prefix"> </slot>
    </template>
    <template #append>
      <ImageButton :size="size" @codeId="getCodeId" class="imgbutton" />
    </template>
  </a-input>
</template>
<script lang="ts" setup>
import ImageButton from './ImageButton.vue'
import { useDesign, useRuleFormItem } from '@anyi/corehooks'
const props = defineProps({
  value: { type: String },
  size: {
    type: String,
    default: 'medium',
    validator: (v: String) => ['mini' | 'small' | 'medium' | 'large'].includes(v as string),
  },
})
const emit = defineEmits(['codeId'])
const { prefixCls } = useDesign('countdown-input')
function getCodeId(codeId: string) {
  emit('codeId', codeId)
}
</script>
<style lang="less">
