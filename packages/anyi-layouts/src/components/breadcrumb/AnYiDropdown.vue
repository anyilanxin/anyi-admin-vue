...
<template>
  <a-doption
    ...
    :value="item.path"
    @click="click(item.path, item)"
  >
    {{ $t(item.meta.title) }}
  </a-doption>
  <a-dsubmenu :value="item.path" v-else>
    <template #default> {{ $t(item.meta.title) }}</template>
    <template #content>
      <AnYiDropdown :item="itemsub" v-for="itemsub in item.children" :click="click" />
    </template>
  </a-dsubmenu>
</template>

<script lang="ts" setup name="AnYiDropdown">
defineProps({
