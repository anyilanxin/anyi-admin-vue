...
<template>
  <template v-if="menuItem?.children && menuItem?.children.length !== 0">
    <a-sub-menu :key="menuItem.name">
      <template #icon>
        <span v-if="getMenuCollapsed && enableCollapsed && mode != MenuModeEnum.HORIZONTAL">
          {{ getFirstItemTitleIcon($t(menuItem?.title || '')) }}
        </span>
        <img
  ...
  <template v-else>
    <a-menu-item :key="menuItem.name" @click="() => goto(menuItem)">
      <template #icon>
        <span
          v-if="
            getMenuCollapsed &&
...
import { useRoute, useRouter, RouteRecordRaw } from '@anyi/router'
import { regexUrl, openWindow } from '@anyi/coreutils'
import { ref } from 'vue'
import { RouterLink } from '@anyi/router'
const { menu, sidebar, isMixSidebar, getCollapsedShowTitle, isTopMenu } = useAppConfig()
